...
{
    public class MortonEncodingTest_2D
    {
        // LUT for Morton2D encode X
        static UInt32[] morton256_2d_x = new UInt32[256]
        {
            0, 1, 4, 5, 16, 17, 20, 21,
        ...
        static UInt32[] morton256_2d_y = new UInt32[256]
        {
        ...
        private UInt64 mortonEncode_LUT(UInt32 x, UInt32 y)
        {
            UInt64 answer = 0;
            answer =
                morton256_2d_y[(y >> 16) & 0xFF]// we start by shifting the third byte, since we only look at the first 21 bits
                | morton256_2d_x[(x >> 16) & 0xFF];
            answer = answer << 32
                     | morton256_2d_y[(y >> 8) & 0xFF]   // shifting second byte
                     | morton256_2d_x[(x >> 8) & 0xFF];
            answer = answer << 16 |
                     morton256_2d_y[(y) & 0xFF] |
                     morton256_2d_x[(x) & 0xFF];
            return answer;
        }
        ...
        public void Encode_XY_Int32_LookupTable()
        {
            ...
            Parallel.For(0, max, (i) =>
            {
                ...
                {
                    var a = mortonEncode_LUT(x, y);

                    // Encode test
                    UInt32 r = Tedd.MortonEncoding.Encode(x, y);
                    Assert.Equal(a, r);

                    // Decode test
                    Tedd.MortonEncoding.Decode(r, out var rx, out var ry);
                    Assert.Equal(x, rx);
                    Assert.Equal(y, ry);
                }
            });
